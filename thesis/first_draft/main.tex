...
stable, omnidirectional movement on a machine driven by position controlled
...
For the movemen a trot gait developed, which consists of the swing phase that
follows a Bezier curve whose repeated end control points give zero foot
velocity and acceleration at liftoff and touchdown, and the stance phase
that follows a sinusoidal profile with a controlled ground penetration depth. The
gait is extended to omnidirectional motion by composing a linear step
...
Orientation is stabilised in a closed loop. Two independent PID controllers act
...
trajectories rather than through the body pose. The PID controller input comes from
...
before reaching the PID controller.
...
while moving, and keeps its frame relatively level through the stabilisation loop, with a
...
όπως θα δούμε στην ενότητα~\ref{sec:joint-space-traj}. Για τον χειρισμό της
στάσης σε πραγματικό χρόνο υλοποιήθηκε επιπλέον σταδιακός έλεγχος με
βαθυπερατό φίλτρο (low pass filter) πρώτης τάξης: σε κάθε βήμα ελέγχου η στάση
του σώματος $\boldsymbol{\eta} = [\omega\ \ \psi\ \ \phi]^{T}$ πλησιάζει την
επιθυμητή $\boldsymbol{\eta}^{*}$ κατά ένα σταθερό ποσοστό της απόκλισης,
\begin{equation}
\boldsymbol{\eta}_{k} = \boldsymbol{\eta}_{k-1} +
\kappa\left(\boldsymbol{\eta}^{*} - \boldsymbol{\eta}_{k-1}\right),
\qquad \kappa = 0{.}1
\label{eq:body-lowpass}
\end{equation}
ώστε το σώμα να ακολουθεί την εντολή χωρίς απότομες κινήσεις.
...
οι μεταβάσεις εκτελούνται με το ρομπότ ακίνητο και με διάρκεια $T = 1{,}5$~s
κατά την εκκίνηση και την επιστροφή στη στάση ανάπαυσης. Η συνέχεια και της επιτάχυνσης αποτελεί πρόσθετη απαίτηση που
...
\begin{figure}[htbp]
    ...
    άρθρωσης κατά $30^\circ$ σε $T = 0{,}5$~s, ενδεικτικά (στην εκκίνηση
    χρησιμοποιείται $T = 1{,}5$~s). (α) Γωνία $q(t)$ με το
...
άξονα, δίνονται στον Πίνακα~\ref{tab:pid-gains}. Στον άξονα roll ο
ολοκληρωτικός όρος απενεργοποιήθηκε ($K_i = 0$), οπότε ο ελεγκτής λειτουργεί ως
αναλογικός διαφορικός (PD).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l l c c@{}}
        ...
        \midrule
        Αναλογικό κέρδος & $K_p$ & $0{,}50$ & $1{,}00$ \\
        Ολοκληρωτικό κέρδος & $K_i$ & $0$ & $0{,}02$ \\
        Διαφορικό κέρδος & $K_d$ & $0{,}05$ & $0{,}01$ \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l l p{8.4cm}@{}}
        ...
        \midrule
        \texttt{config/} & \texttt{robot\_config.yaml} & Διαστάσεις σκελετού και μηκών συνδέσμων, όρια αρθρώσεων, σημεία ελέγχου Bezier, συντελεστές διείσδυσης και κέρδη PID \\
        ...
        & \texttt{gotorest.py} & Μετάβαση στη στάση ανάπαυσης \\
        \midrule
        ...
        \midrule
        \texttt{log/} & \texttt{pid\_plotter.py} & Γραφήματα από τα αρχεία καταγραφής \\
        & \texttt{pid\_plotter.m} & Αντίστοιχη υλοποίηση σε MATLAB \\
        & \texttt{imu\_plotter.py} & Γραφήματα γωνιών IMU πριν και μετά το βαθυπερατό φίλτρο \\
        & \texttt{pid/} & Αρχεία καταγραφής του ελεγκτή σταθεροποίησης \\
        & \texttt{imu/} & Αρχεία καταγραφής της αδρανειακής μονάδας \\
        \bottomrule
    \end{tabular}
...
αντιμετωπίστηκαν ως επιπλέον θόρυβος, ο οποίος εξομαλύνεται στην έξοδο με ένα
εκθετικό βαθυπερατό φίλτρο (low pass filter), το οποίο σταθμίζει κάθε νέα
μέτρηση με την προηγούμενη εξομαλυμένη τιμή,
\begin{equation}
...
Με $\alpha = 0{,}3$ και βήμα ελέγχου περίπου 15~ms, η σταθερά χρόνου του
φίλτρου είναι περίπου 12~ms και η συχνότητα αποκοπής περίπου 13~Hz. Το φίλτρο
αφήνει ανέπαφη τη διακύμανση του βαδίσματος (κάτω από 5~Hz) και περιορίζει
κυρίως τον θόρυβο υψηλής συχνότητας, ο οποίος ενισχύεται από τον διαφορικό όρο
του ελεγκτή.
...
μοίρα, καθώς και ο θόρυβος υψηλής συχνότητας στην έξοδο του ελεγκτή PID, ο
οποίος περιορίζεται με το φιλτράρισμα.
...
\begin{figure}[htbp]
    ...
    του ελεγκτή. (β) Με το βαθυπερατό φίλτρο της \eqref{eq:lowpass}}
    \label{fig:sensor-noise-plot}
\end{figure}

Επειδή το Σχήμα~\ref{fig:sensor-noise-plot} συγκρίνει δύο διαφορετικές
εκτελέσεις, η επίδραση του φίλτρου μετρήθηκε και από το αρχείο καταγραφής της
αδρανειακής μονάδας (ενότητα~\ref{sec:data-logging}), στο οποίο το σήμα πριν και
μετά το φίλτρο καταγράφεται στην ίδια εκτέλεση. Σε εκτέλεση με το ρομπότ
αιωρούμενο, η ισχύς που αφαιρεί το φίλτρο σε κάθε ζώνη συχνοτήτων δίνεται στον
Πίνακα~\ref{tab:lpf-bands}, ενώ η ενεργός τιμή (RMS) της διαφοράς των δύο
σημάτων είναι μόλις $0{,}02$ έως $0{,}03^\circ$. Με το ρομπότ αιωρούμενο δεν
υπάρχουν κρούσεις των πελμάτων στο έδαφος, οπότε οι τιμές αυτές υποεκτιμούν την
επίδραση του φίλτρου κατά το βάδισμα πάνω στο έδαφος.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l c@{}}
        \toprule
        Ζώνη συχνοτήτων & Ισχύς που αφαιρείται \\
        \midrule
        0 έως 5~Hz (κίνηση του βαδίσματος) & $< 1\%$ \\
        5 έως 15~Hz & 25 έως 33\% \\
        15 έως 34~Hz & 61 έως 64\% \\
        \bottomrule
    \end{tabular}
    \caption[Επίδραση του βαθυπερατού φίλτρου ανά ζώνη συχνοτήτων]{Ποσοστό της
    ισχύος του σήματος που αφαιρεί το βαθυπερατό φίλτρο σε κάθε ζώνη
    συχνοτήτων, από εκτέλεση με το ρομπότ αιωρούμενο}
    \label{tab:lpf-bands}
\end{table}
...
Ο βρόχος έχει ονομαστική περίοδο 10~ms (100~Hz). Στην πράξη ένα βήμα ελέγχου
διαρκεί περίπου 15~ms, οπότε η μετρημένη συχνότητα είναι περίπου 67~Hz. Το
μεγαλύτερο μέρος του χρόνου καταναλώνεται στον δίαυλο
I\textsuperscript{2}C, ο οποίος λειτουργεί στην προεπιλεγμένη ταχύτητα των
100~kHz: οι δώδεκα εντολές θέσης προς τους οδηγούς PCA9685 απαιτούν περίπου
9,5~ms και η ανάγνωση του επιταχυνσιομέτρου περίπου 2,8~ms. Επειδή ο ελεγκτής
PID χρησιμοποιεί το πραγματικό χρονικό βήμα $\Delta t_k$ της
\eqref{eq:pid-output} και η φάση του βαδίσματος προκύπτει από το ρολόι του
συστήματος, η χαμηλότερη συχνότητα δεν μεταβάλλει ούτε τη δράση των κερδών ούτε
τον χρονισμό του βαδίσματος.

Ως μέτρο ασφαλείας, αν η απόλυτη γωνία pitch ξεπεράσει τις $45^\circ$, το βήμα
ελέγχου δεν εκτελείται και ο βρόχος επιστρέφει σε ηρεμία. Ο έλεγχος αφορά μόνο
το pitch, οπότε πτώση στο πλάι δεν ανιχνεύεται.
...
\texttt{pydualsense}. Ο βρόχος του τηλεχειρισμού έχει ονομαστική συχνότητα
100~Hz (μετρημένη περίπου 67~Hz, ενότητα~\ref{sec:realtime-control}): σε κάθε
...
\begin{figure}[htbp]
    ...
    τηλεχειρισμού στα χειριστήρια του DualSense. Οι προσκρουστήρες L1 και R1
    βρίσκονται στην πάνω πλευρά και οι σκανδάλες L2 και R2 ακριβώς πίσω τους,
    οπότε δεν φαίνονται στη φωτογραφία. Πηγή της φωτογραφίας: Sony Interactive
    Entertainment}
...
$[-128,\ 128]$. Ο μοχλός θεωρείται ενεργός μόνο όταν η απόκλισή του από το
...
Ο δεξιός μοχλός χρησιμοποιείται για το χειρισμό Roll και Pitch γωνιών του σώματος όσο το ρομπότ είναι
ακίνητο. Οι δύο άξονές του κανονικοποιούνται διαιρώντας με 128, ώστε να
προκύπτουν τιμές στο διάστημα $[-1,\ 1]$, και πολλαπλασιάζονται με τη μέγιστη
επιτρεπόμενη γωνία,
\begin{equation}
\omega^{*} = -\frac{x_R}{128}\,\omega_{max}, \qquad
\psi^{*} = \frac{y_R}{128}\,\psi_{max}, \qquad
\omega_{max} = 15^\circ, \quad \psi_{max} = 10^\circ,
\label{eq:joystick-tilt}
\end{equation}
όπου $\omega^{*}$ και $\psi^{*}$ η επιθυμητή γωνία roll και pitch αντίστοιχα. Η
στάση που προκύπτει εφαρμόζεται μέσω του σταδιακού ελέγχου της
\eqref{eq:body-lowpass}.
...
δίνεται γωνιακή ταχύτητα $-0{,}5$~rad/s με το R1 (δεξιά) και $+0{,}3$~rad/s με το
L1 (αριστερά). Οι τιμές αυτές επιλέχθηκαν πειραματικά. Στη στροφή κατά το βάδισμα, με τις σκανδάλες R2 και L2, η γραμμική ταχύτητα
παραμένει 0,12~m/s και η γωνιακή είναι $-0{,}5$ και $+0{,}5$~rad/s αντίστοιχα
...
ανά βήμα ελέγχου, δηλαδή με τη μετρημένη συχνότητα του βρόχου, περίπου 67~Hz
(ενότητα~\ref{sec:realtime-control}). Κάθε γραμμή
...
αρχείο CSV δίνεται στη μονάδα \texttt{pid\_plotter.py}, η οποία μετατρέπει τις
...
και η αντίστοιχη έξοδος του ελεγκτή. Ο άξονας του χρόνου ξεκινά από τα 0~s,
αφαιρώντας την πρώτη χρονική στιγμή του αρχείου. Το γράφημα αποθηκεύεται ως
εικόνα δίπλα στο αρχείο δεδομένων, με το ίδιο όνομα, όπως στο
Σχήμα~\ref{fig:log-plotter-sample}. Διατίθεται και ισοδύναμη υλοποίηση σε
MATLAB (\texttt{pid\_plotter.m}) για την ίδια μορφή αρχείου.

Παράλληλα καταγράφεται σε δεύτερο αρχείο (\texttt{log/imu/}) η έξοδος του
φίλτρου σύντηξης πριν και μετά το βαθυπερατό φίλτρο, με στήλες
\begin{center}
\texttt{t, imu\_roll, imu\_pitch, imu\_yaw, lpf\_roll, lpf\_pitch, lpf\_yaw}
\end{center}
όπου ο χρόνος μετράται σε δευτερόλεπτα από την εκκίνηση και οι γωνίες σε
ακτίνια. Η μονάδα \texttt{imu\_\allowbreak plotter.py} παράγει γράφημα τριών αξόνων (roll,
pitch, yaw).

Πριν από κάθε νέα εκτέλεση τα προηγούμενα αρχεία δεδομένων και εικόνων
διαγράφονται, ώστε στους φακέλους καταγραφής να παραμένει πάντα μόνο η τελευταία δοκιμή. Με
...
\begin{figure}[htbp]
    ...
    γραφήματος που παράγει η \texttt{pid\_plotter.py} στο τέλος κάθε
...
Με τα τελικά κέρδη του Πίνακα~\ref{tab:pid-gains} πραγματοποιήθηκαν δύο ακόμα
δοκιμές στο υλικό, μία με μικτούς βηματισμούς (ευθεία, στροφές, πλάγια κίνηση)
και μία αμιγώς ευθύγραμμη, των οποίων οι στατιστικές τιμές δίνονται στον
Πίνακα~\ref{tab:controller-stats}.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l r c c@{}}
        \toprule
        Δοκιμή & Διάρκεια & Roll ($^\circ$) & Pitch ($^\circ$) \\
        \midrule
        Μικτοί βηματισμοί & 27,6~s & $-2{,}6 \pm 3{,}0\ [-11{,}3,\ 6{,}7]$ & $-1{,}0 \pm 1{,}4\ [-5{,}6,\ 3{,}2]$ \\
        Ευθεία γραμμή & 11,7~s & $-6{,}0 \pm 2{,}5\ [-10{,}6,\ -0{,}6]$ & $-1{,}8 \pm 0{,}8\ [-3{,}9,\ 1{,}1]$ \\
        \bottomrule
    \end{tabular}
    \caption[Στατιστικά της απόκρισης του ελεγκτή στο υλικό]{Μέση τιμή, τυπική
    απόκλιση και εύρος τιμών (σε αγκύλες) των γωνιών roll και pitch, με τα
    τελικά κέρδη του Πίνακα~\ref{tab:pid-gains}}
    \label{tab:controller-stats}
\end{table}

Στο pitch η έξοδος του ελεγκτή ακολουθεί τη μέτρηση με συντελεστή συσχέτισης
0,98 και παραμένει φραγμένη σε όλους τους τύπους βαδίσματος.
...
(ενότητα~\ref{sec:software-architecture}). Playlist με όλα τα demos:
\url{https://youtube.com/playlist?list=PLZkTyBWS-kqA&si=OrUQILE2mhKAoyir}
...
\begin{itemize}
  ...
  \textit{[πραγματικό ρομπότ: \url{https://youtube.com/shorts/qg6DxsJDx6s?is=9vmeTBTlte9AF2FP}]} \quad
  \textit{[προσομοίωση: \url{https://youtu.be/xf1U81D949Y}]}
  \item Στροφή επί τόπου, με μηδενική γραμμική ταχύτητα
  (ενότητα~\ref{sec:turning-gait}).\\
  \textit{[πραγματικό ρομπότ: \url{https://youtube.com/shorts/KCEO29UnrRI?is=_pYyOUsrlYJOz1tU}]} \quad
  \textit{[προσομοίωση: \url{https://youtu.be/p2RyuNh4AjY}]}
  ...
  \textit{[πραγματικό ρομπότ: \url{https://youtube.com/shorts/looKTXwpA2M?is=-R0Yn33CYzdqW9H-}]} \quad
  \textit{[προσομοίωση: \url{https://youtu.be/rEVeohlhA1E}]}
  ...
  \textit{[πραγματικό ρομπότ: \url{https://youtube.com/shorts/2iB1ggFRjvc?is=rrmbS3QypH5pxTui}]} \quad
  \textit{[προσομοίωση: \url{https://youtu.be/JgVbiIq0104}]}
\end{itemize}


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\textwidth]{images/playlist.png}
    \caption[QR code προς το playlist που περιέχει όλα τα βίντεο]{QR code προς
    το playlist που περιέχει όλα τα βίντεο}
    \label{fig:video-qr}
\end{figure}
...
\begin{itemize}
  ...
  προειδοποίηση.
  \item Η διακοπή ασφαλείας ενεργοποιείται μόνο όταν η απόλυτη γωνία pitch
  ξεπεράσει τις $45^\circ$ (ενότητα~\ref{sec:realtime-control}), οπότε πτώση στο
  πλάι δεν ανιχνεύεται.
...
\begin{itemize}
  ...
  \item Επίτευξη της ονομαστικής συχνότητας των 100~Hz του βρόχου ελέγχου
  (ενότητα~\ref{sec:realtime-control}), με τρεις παρεμβάσεις στον δίαυλο
  I\textsuperscript{2}C: αύξηση της ταχύτητάς του στα 400~kHz, ανάγνωση του
  επιταχυνσιομέτρου με μία και όχι με τρεις συναλλαγές, και εγγραφή των θέσεων
  των σερβοκινητήρων σε τμήματα (block writes) των οδηγών PCA9685, δηλαδή τρεις
  συναλλαγές αντί για δώδεκα.
  \item Ανίχνευση πτώσης με βάση τη συνολική κλίση του σώματος και όχι μόνο τη
  γωνία pitch, ώστε να ανιχνεύεται και πτώση στο πλάι.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l l r@{}}
        ...
        Παράμετρος & Σύμβολο & Τιμή \\
        \midrule
        Συχνότητα βρόχου ελέγχου & & 100 Hz (ονομαστική), $\approx 67$ Hz (μετρημένη) \\
        Επιθυμητή γραμμική ταχύτητα & $v$ & 0,12 m/s \\
        Γωνιακή ταχύτητα στροφής επί τόπου & $\omega$ & $-0{,}5$ (R1), $+0{,}3$ (L1) rad/s \\
        Γωνιακή ταχύτητα στροφής κατά το βάδισμα & $\omega$ & $-0{,}5$ (R2), $+0{,}5$ (L2) rad/s \\
        Διάρκεια αιώρησης & $T_{sw}$ & 0,20 s \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l l r@{}}
        ...
        Κέρδη roll & $K_p,\ K_i,\ K_d$ & 0,50; 0; 0,05 \\
        Κέρδη pitch & $K_p,\ K_i,\ K_d$ & 1,00; 0,020; 0,01 \\
